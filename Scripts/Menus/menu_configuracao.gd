...
func _ready() -> void:
	...
	toggle_tracking_color.set_pressed_no_signal(Globais.indicador_direcao_transparente_sem_target)
...
func _on_tracking_color_toggled(toggled_on: bool) -> void:
	Globais.indicador_direcao_transparente_sem_target = toggled_on
...
func _on_button_pressed() -> void:
	if $VBoxContainer/TextEditUsarP1.text.size() > 0:
		Globais.string_pegar_controle_P1 = $VBoxContainer/TextEditUsarP1.text
	if $VBoxContainer/TextEditUsarP2.text.size() > 0:
		Globais.string_pegar_controle_P2 = $VBoxContainer/TextEditUsarP2.text
	if $VBoxContainer/TextEditDropP1.text.size() > 0:
		Globais.string_dropar_controle_P1 = $VBoxContainer/TextEditDropP1.text
	if $VBoxContainer/TextEditDropP2.text.size() > 0:
		Globais.string_dropar_controle_P2 = $VBoxContainer/TextEditDropP2.text
